...
- **Model-server backends (`mlx-server.sh`)** — roster entries may set `backend: "mlx_vlm"` (+ `draft_model`/`draft_kind`/`draft_block_size`); the switcher writes `~/.hermes/dashboard/server-backend` (JSON) and mlx-server.sh execs `~/.hermes/mlx-vlm-venv/bin/python mlx-vlm-launch.py` (mlx_vlm.server, OpenAI-compatible, uvicorn) instead of `python3 -m mlx_lm server`; missing venv → silently falls back to mlx-lm. The venv is ISOLATED (`install-mlx-vlm-venv.sh`: mlx-vlm 0.6.14 + mlx 0.32.1 + transformers 5 — never into the framework Python, it breaks mlx-lm). Qwen3.8-27B runs there with its NATIVE MTP drafter `mlx-community/Qwen3.8-27B-MTP-bf16` (0.9GB; the `-MTP-4bit` drafter ships NaN weights, mlx-vlm #1931) → speculative decoding: M5 Max 31 → 63 tok/s code / 47 prose (block 3; block 6 is SLOWER than AR; ~88%/55% acceptance), Hermes drill cases ~1.4-1.8x faster. `APC_ENABLED=1` (+`APC_EXACT_CACHE_ENTRIES=6`) = exact prefix cache — hybrid SSM models use "exact" whole-prefix snapshots, ~64KB/token; the ~18k-token Hermes system prompt goes 26s cold → 0.4s cached. Prefill is compute-bound at ~630-690 tok/s regardless of `--prefill-step-size` — first turn of a fresh session pays ~25s, nothing else does. `mlx-vlm-launch.py` shims: (1) mlx≥0.32 made `mx.random.state` read-only → mlx-vlm 0.6.14 crashed on every temperature>0 speculative request (`_restore_rng_state`); the launcher no-ops the restore (only RNG-stream separation, sampling stays correct); (2) `MLX_VLM_DEFAULT_REASONING_EFFORT` env → default `reasoning_effort` when thinking is on (template default xhigh); (3) atexit `os._exit(0)` because mlx 0.32 segfaults in the CompileCache destructor at teardown (that's the "Python quit unexpectedly" dialog — harmless but noisy). Thinking toggle on this backend = `--enable-thinking --thinking-budget 8192` + effort low. `_mlx_footprint_gb` pgrep matches both backends. mlx_vlm's `/v1/models` also lists every cached model; Hermes must send the exact model id (both backends load whatever id the request names). Downloads run in a bg thread via `huggingface_hub.snapshot_download`. Rationale: 30B MoE resident ~18GB; a dense 8B (~5GB) is plenty for tool-calling since Claude does the coding.
  ...
  **MTP does NOT disable continuous batching (#25 answered).** `mlx_vlm/server/generation.py` `ResponseGenerator._run_impl()` (identical in 0.6.14 L1745 / 0.6.17 L1779): `if self.draft_model is not None and self.draft_kind != "mtp": self._run_speculative(); return` — MTP *falls through* to the `BatchGenerator` loop (there is even MTP-specific batch coalescing below it), the opposite of mlx-lm's `is_batchable=False`; `/health` reports `continuous_batching_enabled: true` with the drafter loaded. Nothing queues: two streams both start within ~0.5s. **But MTP is a single-stream win and a two-stream LOSS**: under 2-way load each stream collapses to **21.2 tok/s (42.5 aggregate)** vs **59.8 aggregate with no drafter** — ~29% worse, because speculative decoding and batching compete for the same GPU and the drafter's wasted work is charged twice. Keep it: the primary lane is effectively single-user (one human, one surface at a time) and solo MTP is ~47.8 tok/s vs 30.6; the concurrency that would flip the trade (briefings, watchtower, For-You) is exactly what the 9B background lane already absorbs.
  **Shim verdicts (2026-09-04):** (1) `_patch_rng_restore` can never fire in THIS configuration — `_restore_rng_state` is reached only via `_SpeculativeSamplerRNG`, constructed only by `_run_speculative()`, which MTP does not take (gate above); verified 3/3 sampled requests succeed with the patch commented out on both 0.6.14 and 0.6.17. It is insurance for a NON-MTP drafter (DFlash/EAGLE-3) only. Upstream fixed the bug in 0.6.17 (`mx.random.state[i][:] = value`), but the launcher's probe is stale and would keep no-opping the now-working function — delete the probe if 0.6.17 ever ships. (2) The `atexit`/`os._exit(0)` teardown shim is **still load-bearing on mlx 0.32.1**: SIGINT A/B without it exits `-11` with a real `EXC_BAD_ACCESS … CompileCache::CacheEntry::~CacheEntry()` crash report; mlx **0.32.2 fixes it** (exit 0, no report), so it dies with the same upgrade. SIGTERM cannot tell the difference either way (uvicorn shuts down, then the process ends by signal before module teardown).
  **Open question — temperature may be ignored on this path.** With `temperature=1.0, top_p=0.95` and the MTP drafter loaded, three identical prompts returned **byte-identical completions** on both 0.6.14 and 0.6.17, drafter or not. The server appears to sample greedily (or reseed identically per request) on the batch path, i.e. the dashboard's Thinking/temperature knob may not do what it claims. Not investigated — backlog item.
...
- **MLX memory ceiling (stability)** — the KV/prompt cache can balloon under
  ...
  at/above `MLX_SOFT_GB` (50 here); `memory_guard_loop` polls every 30s and above
  `MLX_HARD_GB` (56 here) does a `_mlx_restart()` (bootout→bootstrap — NOT kickstart -k)
  ...
  **RAM-AWARE DEFAULTS (1.0.3)** — both numbers used to be hardcoded for this 64GB
  Mac, which on a 16GB Air means a soft ceiling of 50 that never engages while the
  machine swaps to death. `_machine_ram_gb()` reads `hw.memsize` once (GiB, i.e. the
  capacity Apple prints on the box; falls back to 64 if sysctl fails) and the PURE,
  unit-tested `_mem_ceilings(ram_gb, env)` derives the pair:
  **RAM ≥ 64 GB → (50, 56)** — byte-identical to the old constants, and FLAT rather
  than a percentage because the ceiling bounds the KV-cache balloon, whose useful
  size is set by the workload (~2GB per ~20k-token sequence), not by installed RAM;
  **below 64 GB → soft = round(0.72 × RAM), hard = round(0.82 × RAM)**, with
  **hard ≥ soft + 2** and a floor of **(8, 10)**. So 16→(12,14), 24→(17,20),
  32→(23,26), 36→(26,30), 48→(35,39). `MLX_SOFT_GB`/`MLX_HARD_GB` still win and are
  taken verbatim (a soft-only override drags hard to soft+2; unparseable values are
  ignored rather than raising at import). Same idea in the shell: `mlx-server.sh`
  and `mlx-server-bg.sh` read `hw.memsize` and tier `APC_EXACT_CACHE_ENTRIES` +
  `--prompt-cache-size/-bytes` (primary 6/8GB ≥64, 4/5GB ≥48, 3/3.5GB ≥36, else
  2/2GB; the bg lane has its own lower table anchored on today's 4/3GB because it
  shares the Mac with the ~19GB primary). `models_payload().mem.machine_gb` carries
  the number to the UI, and each roster row gains `fit: "ok"|"tight"|"no"`
  (`_model_fit`: no if `ram` > 0.85 × machine, tight if > 0.60 ×) which the model
  menu renders as "needs ~19 GB · this Mac has 64 GB" — muted, `--warn` when tight,
  `--bad` plus an inert row (`onModelClick` refuses it, not just CSS) when it cannot
  fit, so a user never starts a 17GB download for a model their Mac cannot load.
